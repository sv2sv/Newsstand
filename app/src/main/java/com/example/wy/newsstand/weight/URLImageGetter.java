package com.example.wy.newsstand.weight;
...
public class URLImageGetter implements Html.ImageGetter {
    ...
    private static final String mFilePath = WYNSDepend.getAppContext().getCacheDir().getAbsolutePath();
    ...
    private Drawable getDrawableFromNet(final String source) {
        mSubscription = RetrofitManager.getInstance(Host.NEWS_DETAIL_HTML_PHOTO)
                ...
                .map(new Func1<ResponseBody, Boolean>() {
                    ...
                    public void onCompleted() {
                        Log.i("URLImageGetter", "onCompleted");
                    }

                    @Override
                    public void onError(Throwable e) {
                        Log.e("URLImageGetter", "onError "+ e.toString());
                    }

                    @Override
                    public void onNext(Boolean isLoadSuccess) {
                        Log.i("URLImageGetter", "onNext ");
                        mPicCount++;
    ...
    private Boolean WritePicToDisk(ResponseBody response, String source) {
        ...
        try {
            ...
            Log.e("WritePicToDisk",  e.toString());
            return false;
        } finally {
            try {
                if (in != null) {
                ...
                Log.e("WritePicToDisk", "finally"+ e.toString());
            }
        }
    }
    ...
    private Drawable createPicPlaceholder() {
        Drawable drawable;
        int color = WYNSDepend.getColor(R.color.image_place_holder, R.color.image_place_holder_night);
        drawable = new ColorDrawable(WYNSDepend.getAppContext().getResources().getColor(color));
